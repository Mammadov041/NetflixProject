...
const MovieDetails = () => {
  ...
  const {id, media_type} = route.params; // Get ID & type (movie or TV show)
  ...
  const getTrailerAsync = useCallback(async () => {
    try {
      ...
      if (response.trailers?.length > 0) {
        setTrailer(response.trailers[0]); // Only store the first trailer
      }
    } catch (error) {
    ...
    <ScrollView className="flex-1 bg-black ">
      ...
      <View className="p-5 gap-3">
        ...
        <Text className="text-white font-montSerrat text-lg">
          IMDB Rating : {movie.vote_average.toString().substring(0, 3)}
        </Text>
